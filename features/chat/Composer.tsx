...
export default function Composer({ language, levels, busy, searchAllowed, onSend, onStop }: { language: Language; levels: Level[]; busy: boolean; searchAllowed: boolean; onSend: (value: { text: string; level: string; image?: string; search: boolean }) => void; onStop: () => void }) {
  ...
  return <div className="composer-wrap"><div className="composer">
    ...
    <textarea value={text} onChange={(e) => setText(e.target.value)} placeholder={t.ask} rows={2} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); submit(); } }} />
    <div className="composer-actions"><div className="composer-options"><select value={level} onChange={(e) => setLevel(e.target.value)}><option value="">{t.level}</option>{levels.map((item) => <option key={item.code} value={item.code}>{item.label}</option>)}</select><input ref={fileRef} type="file" hidden accept="image/png,image/jpeg,image/webp" onChange={(e) => void chooseFile(e.target.files?.[0])} /><button title={t.upload} onClick={() => fileRef.current?.click()}><ImagePlus size={18} /></button>{searchAllowed && <button className={search ? 'active-tool' : ''} title={t.webSearch} onClick={() => setSearch(!search)}><Globe2 size={18} /></button>}</div>{busy ? <button className="send-button" onClick={onStop} title={t.stop}><Square size={16} /></button> : <button className="send-button" onClick={submit} title={t.send}><Send size={17} /></button>}</div>
